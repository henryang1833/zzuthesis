...
%%================================================
\chapter{基于特征融合的异常流量检测模型与增量学习实现}
...
\begin{figure}[h]
  \centering
  \includegraphics[width = 0.8\textwidth]{chapter_4.drawio.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[width = 0.8\textwidth]{MultimodalFusion.drawio.png}
...
\begin{table}[htbp]
  ...
  \end{tabularx}
\end{table}
\section{基于残差神经网络与双向门控循环单元多模态特征融合模型实验评估}
\subsection{实验数据集介绍}
...
\section{基于iCaRL的RNB-MF增量学习方案实验评估}
...
% 为验证本文方案进行增量学习的效果，利用ResNet、RNM-NMF、RNB-MF with iCaRL(基于iCaRL的RNB-MF模型)以及RNB-MF with iCaRL and GA(基于遗传算法记忆集抽样优化的RNM-MF with iCaRL)增量学习完所有批次的数据后利用训练好的模型在新任务测试集上进行了准确率测试。
